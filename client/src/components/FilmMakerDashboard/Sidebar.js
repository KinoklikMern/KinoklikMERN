...
export default function Sidebar(props) {
  ...
  return (
    <>
      <nav className="tw-flex tw-h-full tw-w-24 tw-flex-col tw-justify-between tw-gap-3 tw-rounded-lg tw-bg-[#fff] tw-py-16">
        {sideBarList.map((item) => (
          <div className={"tw-flex tw-justify-center hover:tw-scale-105 "+(SELECTED_TAB===item.Title?"tw-bg-[#1E0039]":"")}>
            <a
              href={item.href}
            ...
            >
              <div className="tw-flex tw-justify-center ">
                <img
                  src={SELECTED_TAB===item.Title?item.ActiveIcon:item.DefaultIcon}
                  alt={item.Title}
                  style={{"width":item.size.width, "height":item.size.height}}
                />
              </div>
              <p className={"tw-flex tw-justify-center tw-text-sm " + (SELECTED_TAB===item.Title?"tw-text-white":"")}>
                {item.Title}
              </p>
            </a>
          </div>
