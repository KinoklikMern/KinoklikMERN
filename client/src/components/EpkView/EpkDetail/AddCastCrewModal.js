...
export default function AddCastCrewModal({ 
  ...
  useEffect(() => {
    ...
    const formattedResults = filtered.map((u) => {
      ...
      return {
        ...u,
        id: u._id,
        name: `${u.firstName || ''} ${u.lastName || ''}`.trim() || t('Unknown User'),
        role: u.role || 'User', 
        image: imageUrl,
  ...
  const handleInviteSubmit = () => {
    ...
    if (!inviteData.firstName || !inviteData.lastName || !inviteData.email || !inviteData.role) {
      setInviteError(t("Please fill out all fields."));
      return;
    }
    const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!regex.test(inviteData.email)) {
      setInviteError(t("Please enter a valid email."));
      return;
    }
    const userExists = allUsers.find((user) => user.email.toLowerCase() === inviteData.email.toLowerCase());
    if (userExists) {
      setInviteError(`${t("User with that email is already registered as:")} ${userExists.firstName} ${userExists.lastName}. ${t("Please search for their name instead.")}`);
      return;
    }
  ...
  return (
    <div className="tw-fixed tw-inset-0 tw-z-[9999] tw-flex tw-items-center tw-justify-center tw-bg-[#190033]/90 tw-backdrop-blur-md md:tw-p-4">
      ...
      >
        ...
        <h1 className="tw-font-bold tw-text-2xl tw-text-[#FF43A7] tw-tracking-tight tw-mb-8">{t("Cast & Crew")}</h1>

        {/* SEARCH SECTION */}
        <section className="tw-mb-10">
          <label className="tw-text-[#FFB0CF] tw-text-[10px] tw-uppercase tw-tracking-[0.2em] tw-mb-2 tw-block tw-ml-1">{t("Talent Discovery")}</label>
          <div className="tw-relative tw-w-full tw-mb-4" onClick={(e) => e.stopPropagation()}>
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder={t("Search talent pool...")}
          ...
          {isInviteMode ? (
            <div className="tw-bg-[#371E51]/40 tw-border tw-border-[#5A3F49]/40 tw-rounded-xl tw-p-4" onClick={(e) => e.stopPropagation()}>
              <div className="tw-flex tw-justify-between tw-mb-4">
                ...
                <button onClick={() => setIsInviteMode(false)} className="tw-text-[#AA8894] tw-bg-transparent tw-border-none tw-underline tw-text-xs">{t("Cancel")}</button>
              </div>
              <div className="tw-flex tw-gap-2 tw-mb-3">
                <input type="text" placeholder={t("First Name")} value={inviteData.firstName} onChange={(e) => setInviteData({...inviteData, firstName: e.target.value})} className="tw-w-1/2 tw-bg-[#190033] tw-border tw-border-[#5A3F49]/40 tw-rounded-lg tw-px-3 tw-py-2 tw-text-white tw-text-sm focus:tw-outline-none focus:tw-border-[#FF43A7]" />
                <input type="text" placeholder={t("Last Name")} value={inviteData.lastName} onChange={(e) => setInviteData({...inviteData, lastName: e.target.value})} className="tw-w-1/2 tw-bg-[#190033] tw-border tw-border-[#5A3F49]/40 tw-rounded-lg tw-px-3 tw-py-2 tw-text-white tw-text-sm focus:tw-outline-none focus:tw-border-[#FF43A7]" />
              </div>
              <input type="email" placeholder={t("Email")} value={inviteData.email} onChange={(e) => setInviteData({...inviteData, email: e.target.value})} className="tw-w-full tw-bg-[#190033] tw-border tw-border-[#5A3F49]/40 tw-rounded-lg tw-px-3 tw-py-2 tw-mb-3 tw-text-white tw-text-sm focus:tw-outline-none focus:tw-border-[#FF43A7]" />
              <select value={inviteData.role} onChange={(e) => setInviteData({...inviteData, role: e.target.value})} className="tw-w-full tw-bg-[#190033] tw-border tw-border-[#5A3F49]/40 tw-rounded-lg tw-px-3 tw-py-2 tw-mb-4 tw-text-white tw-text-sm focus:tw-outline-none focus:tw-border-[#FF43A7]">
                ...
                {epkRoles.map(r => <option key={r} value={r}>{t(r.toLowerCase().replace(/ /g, "_"))}</option>)}
              </select>
              {inviteError && <p className="tw-text-red-400 tw-text-xs tw-mb-3">{inviteError}</p>}
              <button onClick={handleInviteSubmit} className="tw-w-full tw-bg-[#FF43A7] tw-text-[#570033] tw-font-bold tw-py-3 tw-rounded-lg tw-border-none">{t("Send Invitation")}</button>
            </div>
          ) : (
                  ...
                  <div key={user.id} className="tw-flex tw-items-center tw-justify-between tw-p-3 hover:tw-bg-[#371E51] tw-rounded-xl tw-transition-colors">
                    <div className="tw-flex tw-items-center tw-gap-4">
                      ...
                      <div>
                        ...
                        <p className="tw-text-[10px] tw-text-[#DDB7FF]/70 tw-uppercase tw-tracking-wider">{t(String(user.role).toLowerCase().replace(/ /g, "_"))}</p>
                      </div>
                    </div>
                    {user.alreadyAdded || user.alreadyInvited ? (
                       <span className="tw-text-[#FFB0CF]/50 tw-text-[10px] tw-uppercase tw-font-bold">{t("Added")}</span>
                    ) : (
          ...
          {searchQuery && searchResults.length === 0 && !isLoading && !isInviteMode && (
            <button onClick={(e) => { e.stopPropagation(); setIsInviteMode(true); setInviteData({...inviteData, email: searchQuery.includes('@') ? searchQuery : ''}); }} className="tw-w-full tw-mt-2 tw-bg-transparent tw-border tw-border-[#FF43A7] tw-text-[#FFB0CF] tw-py-3 tw-rounded-xl tw-text-sm tw-font-bold tw-cursor-pointer">
              {t("Invite")} "{searchQuery}" {t("via Email")}
            </button>
          )}
        </section>
        ...
        <section className="tw-mb-10">
          <div className="tw-flex tw-items-baseline tw-gap-3 tw-mb-4">
            <h2 className="tw-font-bold tw-text-2xl tw-tracking-tighter tw-text-white">{t("CAST")}</h2>
              ...
              return (
                <div key={idx} onClick={(e) => handleMobileCardClick(e, uId)} className="tw-flex-shrink-0 tw-w-20 tw-text-center tw-space-y-2 tw-relative tw-cursor-pointer">
                  ...
                  <div className="tw-space-y-0.5">
                    ...
                    <p className="tw-text-[9px] tw-text-[#FFB0CF] tw-uppercase tw-truncate">{t(String(member.role).toLowerCase().replace(/ /g, "_"))}</p>
                  </div>
                </div>
              );
            })}
              ...
              return (
                <div key={`inv-${idx}`} onClick={(e) => handleMobileCardClick(e, uId)} className="tw-flex-shrink-0 tw-w-20 tw-text-center tw-space-y-2 tw-relative tw-cursor-pointer">
                  <div className="tw-w-20 tw-h-20 tw-rounded-full tw-p-1 tw-bg-yellow-500/30 tw-relative">
                    <div className="tw-w-full tw-h-full tw-rounded-full tw-overflow-hidden tw-border-2 tw-border-[#1F0439] tw-bg-[#371E51] tw-flex tw-items-center tw-justify-center">
                      <span className="tw-text-[8px] tw-text-yellow-500/80 tw-uppercase tw-font-bold tw-tracking-tighter">{t("Invited")}</span>
                    </div>
                    {isSelected && (
                      <button 
                        onClick={(e) => {
                  ...
                  <div className="tw-space-y-0.5">
                    ...
                    <p className="tw-text-[9px] tw-text-yellow-500/70 tw-uppercase tw-truncate">{t("Pending")}</p>
                  </div>
                </div>
              );
            })}
        ...
        <section className="tw-mb-10">
          <div className="tw-flex tw-items-baseline tw-gap-3 tw-mb-4">
            <h2 className="tw-font-bold tw-text-2xl tw-tracking-tighter tw-text-white">{t("CREW")}</h2>
            ...
            {currentCrew.map((member, idx) => {
              ...
              return (
                <div key={idx} onClick={(e) => handleMobileCardClick(e, uId)} className="tw-flex-shrink-0 tw-w-20 tw-text-center tw-space-y-2 tw-relative tw-cursor-pointer">
                  ...
                  <div className="tw-space-y-0.5">
                    <p className="tw-font-bold tw-text-xs tw-text-white tw-truncate">{member.name}</p>
                    <p className="tw-text-[9px] tw-text-[#DDB7FF] tw-uppercase tw-truncate">{t(String(member.role).toLowerCase().replace(/ /g, "_"))}</p>
                  </div>
                </div>
              );
            })}
            ...
            {invitedCrew.map((invite, idx) => {
              ...
              return (
                <div key={`inv-${idx}`} onClick={(e) => handleMobileCardClick(e, uId)} className="tw-flex-shrink-0 tw-w-20 tw-text-center tw-space-y-2 tw-relative tw-cursor-pointer">
                  ...
                  <div className="tw-space-y-0.5">
                    <p className="tw-font-bold tw-text-xs tw-text-white tw-truncate">{invite.firstName}</p>
                    <p className="tw-text-[9px] tw-text-yellow-500/70 tw-uppercase tw-truncate">{t(String(invite.role).toLowerCase().replace(/ /g, "_"))}</p>
                  </div>
                </div>
              );
            })}
      ...
      <div className="tw-hidden md:tw-flex tw-relative tw-w-full tw-max-w-[1024px] tw-h-[640px] tw-bg-[#280D41] tw-border tw-border-[#5A3F49]/30 tw-shadow-[0_30px_60px_rgba(0,0,0,0.8)] tw-rounded-2xl tw-flex-row tw-overflow-hidden">
        ...
        <div className="tw-w-[40%] tw-h-full tw-p-8 tw-border-r tw-border-[#5A3F49]/20 tw-flex tw-flex-col">
          ...
          <div className="tw-flex tw-flex-col tw-mb-6">
            <span className="tw-text-[#FFB0CF] tw-text-[10px] tw-font-bold tw-uppercase tw-tracking-widest tw-mb-2">{t("Management Suite")}</span>
            <h1 className="tw-text-white tw-text-3xl tw-font-extrabold tw-tracking-tight tw-m-0">{t("Search Cast & Crew")}</h1>
          </div>
          ...
          <div className="tw-relative tw-w-full tw-mb-4">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder={isLoading ? t("Loading users...") : t("Start typing name or email...")}
              disabled={isLoading || isInviteMode}
          ...
          {isInviteMode ? (
            <div className="tw-flex-1 tw-flex tw-flex-col tw-bg-[#371E51]/40 tw-border tw-border-[#5A3F49]/40 tw-rounded-xl tw-p-4">
              <div className="tw-flex tw-items-center tw-justify-between tw-mb-4">
                ...
                <button onClick={() => setIsInviteMode(false)} className="tw-text-[#AA8894] hover:tw-text-white tw-bg-transparent tw-border-none tw-cursor-pointer tw-text-xs tw-underline">{t("Cancel")}</button>
              </div>
              
              <div className="tw-flex tw-gap-2 tw-mb-3">
                <input type="text" placeholder={t("First Name")} value={inviteData.firstName} onChange={(e) => setInviteData({...inviteData, firstName: e.target.value})} className="tw-w-1/2 tw-bg-[#190033] tw-border tw-border-[#5A3F49]/40 tw-rounded-lg tw-px-3 tw-py-2 tw-text-white tw-text-sm tw-outline-none focus:tw-border-[#FF43A7]" />
                <input type="text" placeholder={t("Last Name")} value={inviteData.lastName} onChange={(e) => setInviteData({...inviteData, lastName: e.target.value})} className="tw-w-1/2 tw-bg-[#190033] tw-border tw-border-[#5A3F49]/40 tw-rounded-lg tw-px-3 tw-py-2 tw-text-white tw-text-sm tw-outline-none focus:tw-border-[#FF43A7]" />
              </div>
              <input type="email" placeholder={t("Email Address")} value={inviteData.email} onChange={(e) => setInviteData({...inviteData, email: e.target.value})} className="tw-w-full tw-bg-[#190033] tw-border tw-border-[#5A3F49]/40 tw-rounded-lg tw-px-3 tw-py-2 tw-mb-3 tw-text-white tw-text-sm tw-outline-none focus:tw-border-[#FF43A7]" />
            ...
            <div className="tw-flex-1 tw-overflow-y-auto custom-scrollbar tw-flex tw-flex-col tw-gap-3 tw-pr-2">
              ...
              {searchQuery && searchResults.length === 0 && !isLoading && (
                <div className="tw-flex tw-flex-col tw-items-center tw-justify-center tw-h-full tw-text-center tw-gap-4">
                  <p className="tw-text-[#AA8894] tw-text-sm">{t("No users found matching")} "{searchQuery}"</p>
                ...
                <div key={user.id} className={`tw-flex tw-items-center tw-justify-between tw-p-3 tw-rounded-xl tw-border ${user.alreadyAdded || user.alreadyInvited ? 'tw-bg-[#371E51]/20 tw-border-transparent' : 'tw-bg-[#371E51]/40 tw-border-[#5A3F49]/20'}`}>
                  <div className="tw-flex tw-items-center tw-gap-3">
                    ...
                    <div className="tw-flex tw-flex-col">
                      ...
                      <span className="tw-text-[#AA8894] tw-text-xs">{t(String(user.role).toLowerCase().replace(/ /g, "_"))}</span>
                    </div>
                  </div>

                  {user.alreadyAdded ? (
                    <span className="tw-text-[#FFB0CF]/50 tw-text-[10px] tw-font-bold tw-uppercase tw-tracking-tight tw-px-2">{t("Added")}</span>
                  ) : user.alreadyInvited ? (
                    <span className="tw-text-yellow-500/70 tw-text-[10px] tw-font-bold tw-uppercase tw-tracking-tight tw-px-2">{t("Invited")}</span>
                  ) : (
        ...
        <div className="tw-w-[60%] tw-h-full tw-bg-[#190033]/50 tw-p-8 tw-flex tw-flex-col">
          ...
          <div className="tw-flex tw-items-center tw-gap-3 tw-mb-2">
            <h2 className="tw-text-white tw-text-sm tw-font-extrabold tw-uppercase tw-tracking-[1.4px] tw-m-0">{t("Production Team")}</h2>
          ...
          <div className="tw-flex tw-flex-1 tw-overflow-hidden">
            ...
            <div className="tw-w-1/2 tw-h-full tw-pr-4 tw-border-r tw-border-[#5A3F49]/20 tw-flex tw-flex-col">
              <div className="tw-flex tw-items-center tw-gap-2 tw-mb-4">
                ...
                <h3 className="tw-text-[#DDB7FF] tw-text-[11px] tw-font-bold tw-uppercase tw-tracking-[2.2px] tw-m-0">{t("Cast")}</h3>
              </div>
              <div className="tw-flex-1 tw-overflow-y-auto custom-scrollbar tw-flex tw-flex-col tw-gap-3 tw-pr-2">
                  ...
                  <div key={`d-cast-${idx}`} className="tw-flex tw-items-center tw-justify-between tw-p-3 tw-bg-[#371E51]/30 tw-rounded-xl tw-group">
                    <div className="tw-flex tw-items-center tw-gap-3">
                      ...
                      <div className="tw-flex tw-flex-col">
                        ...
                        <span className="tw-text-[#AA8894] tw-text-[11px]">{t(String(actor.role || 'Actor').toLowerCase().replace(/ /g, "_"))}</span>
                      </div>
                    </div>
                  ...
                  <div key={`d-inv-cast-${idx}`} className="tw-flex tw-items-center tw-justify-between tw-p-3 tw-bg-[#190033]/40 tw-border tw-border-yellow-500/20 tw-rounded-xl tw-group">
                    <div className="tw-flex tw-items-center tw-gap-3">
                      <div className="tw-w-10 tw-h-10 tw-rounded-full tw-bg-[#371E51] tw-border tw-border-dashed tw-border-yellow-500/40 tw-flex tw-items-center tw-justify-center tw-overflow-hidden tw-shrink-0">
                        <span className="tw-text-[8px] tw-text-yellow-500/70 tw-uppercase tw-font-bold tw-tracking-tighter">{t("Invited")}</span>
                      </div>
                      <div className="tw-flex tw-flex-col">
                        <span className="tw-text-white/70 tw-text-sm tw-font-bold">{invite.firstName} {invite.lastName}</span>
                        <span className="tw-text-yellow-500/70 tw-text-[10px] tw-uppercase tw-tracking-wider">{t("Pending")}</span>
                      </div>
                    </div>
                ...
                {currentCast.length === 0 && invitedCast.length === 0 && <p className="tw-text-[#AA8894] tw-text-xs tw-italic tw-mt-2">{t("No cast members added.")}</p>}
              </div>
            </div>
            ...
            <div className="tw-w-1/2 tw-h-full tw-pl-4 tw-flex tw-flex-col">
              <div className="tw-flex tw-items-center tw-gap-2 tw-mb-4">
                ...
                <h3 className="tw-text-[#DDB7FF] tw-text-[11px] tw-font-bold tw-uppercase tw-tracking-[2.2px] tw-m-0">{t("Crew")}</h3>
              </div>
              <div className="tw-flex-1 tw-overflow-y-auto custom-scrollbar tw-flex tw-flex-col tw-gap-3 tw-pr-2">
                  ...
                  <div key={`d-crew-${idx}`} className="tw-flex tw-items-center tw-justify-between tw-p-3 tw-bg-[#371E51]/30 tw-rounded-xl tw-group">
                    <div className="tw-flex tw-items-center tw-gap-3">
                      ...
                      <div className="tw-flex tw-flex-col">
                        ...
                        <span className="tw-text-[#AA8894] tw-text-[11px] tw-capitalize">{t(String(crewMember.role || crewMember.epkRole?.replace('_', ' ') || 'Crew').toLowerCase().replace(/ /g, "_"))}</span>
                      </div>
                    </div>
                ...
                {currentCrew.length === 0 && invitedCrew.length === 0 && <p className="tw-text-[#AA8894] tw-text-xs tw-italic tw-mt-2">{t("No crew members added.")}</p>}
              </div>
            </div>
