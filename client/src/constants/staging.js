...
export default function UserSummaryEdit({ data, onChange, errors = {}, clearError }) {
  ...
  // Check for either a general error or the specific "too_long" error
    ...
    <div className="tw-flex tw-flex-col tw-gap-6 tw-w-full">
      ...
      <div className="tw-w-full">
        <textarea
          ...
          // Prevents typing past 100, but allows existing text (like the 163 chars) to stay so they can edit it
          maxLength={100} 
          placeholder="Professional tagline or summary..."
          className={`tw-w-full tw-bg-transparent tw-border-l-2 tw-pl-6 tw-py-1 tw-text-[#E2BDC9] tw-text-xl md:tw-text-2xl tw-italic tw-outline-none tw-resize-none tw-transition-all ${
            hasSummaryError ? 'tw-border-red-500' : 'tw-border-[#FF43A7]/30 focus:tw-border-[#FF43A7]'
          }`}
          rows={2}
        />
        <div className="tw-flex tw-justify-between tw-mt-1">
          <div className="tw-flex tw-flex-col">
            {errors.summary === "required" && (
              <p className="tw-text-red-500 tw-text-[10px] tw-uppercase tw-font-bold">Summary Required</p>
            )}
            {isTooLong && (
              <p className="tw-text-red-500 tw-text-[10px] tw-uppercase tw-font-bold">
                Too long! Remove {summaryLength - 100} characters
              </p>
            )}
          </div>
          
          <span className={`tw-text-[10px] tw-uppercase tw-font-bold ${
            isTooLong ? 'tw-text-red-500' : 'tw-text-[#5A3F49]'
          }`}>
            {summaryLength} / 100
          </span>
        </div>
